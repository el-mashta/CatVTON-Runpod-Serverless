# app_flux.py
# This is the main application file for the FastAPI server, specifically for the FLUX pipeline.
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Server starting up, loading FLUX models...")

    # Get model paths from environment variables set in the Dockerfile.flux
    base_model_path = os.getenv("BASE_MODEL_PATH", "/app/models/FLUX.1-Fill-dev")
    ...
    if not os.path.exists(base_model_path) or not os.path.exists(resume_path):
        logger.error("Model paths not found! Check Dockerfile.flux and model download stage.")
        raise RuntimeError("Could not find models. Aborting startup.")

    try:
        # Initialize the FLUX TryOn Pipeline
        pipeline = FluxTryOnPipeline.from_pretrained(base_model_path)
        pipeline.load_lora_weights(
            os.path.join(resume_path, "flux-lora"),
            weight_name='pytorch_lora_weights.safetensors'
        )
        pipeline.to("cuda", torch.bfloat16)
        logger.info("FluxTryOnPipeline loaded successfully.")
...
async def virtual_tryon(request: TryonRequest):
    """The main endpoint to perform the virtual try-on using the FLUX pipeline."""
    ...
    try:
        ...
        logger.info(f"Running FLUX inference with cloth_type: {request.cloth_type} and seed: {request.seed}")
        result_image = pipeline(
            ...
            guidance_scale=30.0,    # Default from original app_flux.py
            generator=generator
        ).images[0] # Access the image from the output object
