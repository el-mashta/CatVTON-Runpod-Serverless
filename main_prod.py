...
RUNPOD_ENDPOINT_ID = os.getenv("RUNPOD_ENDPOINT_ID") # Ensure this is set for the FLUX S3 endpoint
...
async def tryon(
    ...
    # S3 keys (paths within the bucket)
    ...
    try:
        ...
        # 1. Save and prepare images locally
        ...
        # 2. Upload prepared images to S3
        ...
        # 3. Call the RunPod worker with the S3 keys
        runpod_payload = {
            "person_image_key": s3_person_key,
            "garment_image_key": s3_cloth_key,
            "cloth_type": "upper", # This can be parameterized if needed
            "seed": 42
        }
        ...
        logger.debug(f"[{request_id}] URL: {tryon_url}")
        ...
        result_image_key = result.get('result_image_key')
        if not result_image_key:
             raise ValueError(f"Unexpected output format from worker: {result}")

        # 4. Download the result from S3 to serve it to the user
...
if __name__ == "__main__":
    ...
    uvicorn.run("main_prod:app", host="0.0.0.0", port=8000, reload=True)
